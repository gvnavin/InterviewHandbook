...
public class QuestionView {
	...
	public View getQuestionsView() {
		...
		for (int i = 0; i < mQuestions.length; i++) {
			...
			Spanned sp = Html.fromHtml(mQuestions[i].getQuestion());
			android.util.Log.d("Navin", sp.toString());
			UIUtil.updateText(question, R.id.question_text, sp);
			...
			UIUtil.updateTag(question, R.id.explanation_id, i + "_explanation");
	...
	public void validateAnswers(SparseIntArray mChosenAnswers , View content, Activity actvity) {
		...
		for (int i = 0; i < mQuestions.length; i++) {
			...
			if (chosenAnswerIndex != valueIfKeyNotFound) {
				...
				((TextView) content.findViewWithTag(i + "_explanation")).setText(mQuestions[i].getExplanation());
				...
				UIUtil.makeViewVisible(content, i + "_explanation");		
			...
			UIUtil.makeViewVisible(actvity, R.id.status_bar);
